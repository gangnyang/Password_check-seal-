...
int main() {
	...
	string sk_file = "secret_key.seal"; // For debugging
	...
	try { // SQL 연결
		...
		while (res->next()) {
			...
			for (size_t i = 0; i < rotate_size-1; i++) { // 4글자씩 한 벡터 요소에 담기 때문에 PADDING LENGTH/4 - 1만큼 회전 연산 후 더한다
			...
			*/
			...
			size_t numofpasswords = row_size / rotate_size; // LOG2(비밀번호 수)만큼 회전 후 곱하면 모든 비밀번호의 정보를 담을 수 있다
			size_t numofsteps = log2(numofpasswords); // 근데 노이즈 예산 부족으로 한번만 곱해서 전달함
			while (numofpasswords > PADDING_LENGTH / 4) {
